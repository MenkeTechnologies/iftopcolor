...
dlenabmultireq(fd, addr, length) int fd;
char *addr;
int length;
{
    long buf[MAXDLBUF];
...
dldisabmultireq(fd, addr, length) int fd;
char *addr;
int length;
{
    long buf[MAXDLBUF];
...
dlpromisconreq(fd, level) int fd;
u_long level;
{
...
dlphysaddrreq(fd, addrtype) int fd;
u_long addrtype;
{
...
dlsetphysaddrreq(fd, addr, length) int fd;
char *addr;
int length;
{
    long buf[MAXDLBUF];
...
dlbindreq(fd, sap, max_conind, service_mode, conn_mgmt, xidtest) int fd;
u_long sap;
u_long max_conind;
u_long service_mode;
u_long conn_mgmt;
...
strgetmsg(fd, ctlp, datap, flagsp, caller) int fd;
 ...
 */
printdlprim(dlp) union DL_primitives *dlp;
{
...
/* ARGSUSED */
printdlinforeq(dlp) union DL_primitives *dlp;
{ (void)printf("DL_INFO_REQ\n"); }

printdlinfoack(dlp) union DL_primitives *dlp;
{
    u_char addr[MAXDLADDR];
    u_char brdcst[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->info_ack.dl_addr_offset), dlp->info_ack.dl_addr_length, addr);
    ...
    (void)printf("addr_length %d mac_type %s current_state %s\n", dlp->info_ack.dl_addr_length,
                 dlmactype(dlp->info_ack.dl_mac_type), dlstate(dlp->info_ack.dl_current_state));
    ...
    (void)printf("provider_style %s addr_offset %d version %d\n",
                 dlstyle(dlp->info_ack.dl_provider_style), dlp->info_ack.dl_addr_offset,
...
printdlattachreq(dlp) union DL_primitives *dlp;
{ (void)printf("DL_ATTACH_REQ:  ppa %d\n", dlp->attach_req.dl_ppa); }

printdlokack(dlp) union DL_primitives *dlp;
{ (void)printf("DL_OK_ACK:  correct_primitive %s\n", dlprim(dlp->ok_ack.dl_correct_primitive)); }

printdlerrorack(dlp) union DL_primitives *dlp;
{
    (void)printf("DL_ERROR_ACK:  error_primitive %s errno %s unix_errno %d\n",
                 dlprim(dlp->error_ack.dl_error_primitive), dlerrno(dlp->error_ack.dl_errno),
                 dlp->error_ack.dl_unix_errno);
}

printdlenabmultireq(dlp) union DL_primitives *dlp;
{
    u_char addr[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->enabmulti_req.dl_addr_offset), dlp->enabmulti_req.dl_addr_length,
                 addr);
...
printdldisabmultireq(dlp) union DL_primitives *dlp;
{
    u_char addr[MAXDLADDR];
...
printdlpromisconreq(dlp) union DL_primitives *dlp;
{ (void)printf("DL_PROMISCON_REQ:  level %s\n", dlpromisclevel(dlp->promiscon_req.dl_level)); }

printdlpromiscoffreq(dlp) union DL_primitives *dlp;
{ (void)printf("DL_PROMISCOFF_REQ:  level %s\n", dlpromisclevel(dlp->promiscoff_req.dl_level)); }

printdlphysaddrreq(dlp) union DL_primitives *dlp;
{ (void)printf("DL_PHYS_ADDR_REQ:  addr_type 0x%x\n", dlp->physaddr_req.dl_addr_type); }

printdlphysaddrack(dlp) union DL_primitives *dlp;
{
    u_char addr[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->physaddr_ack.dl_addr_offset), dlp->physaddr_ack.dl_addr_length,
                 addr);
...
printdlsetphysaddrreq(dlp) union DL_primitives *dlp;
{
    u_char addr[MAXDLADDR];
...
/* ARGSUSED */
printdldetachreq(dlp) union DL_primitives *dlp;
{ (void)printf("DL_DETACH_REQ\n"); }

printdlbindreq(dlp) union DL_primitives *dlp;
{
    ...
    (void)printf("service_mode %s conn_mgmt %d xidtest_flg 0x%x\n",
                 dlservicemode(dlp->bind_req.dl_service_mode), dlp->bind_req.dl_conn_mgmt,
                 dlp->bind_req.dl_xidtest_flg);
}

printdlbindack(dlp) union DL_primitives *dlp;
{
    u_char addr[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->bind_ack.dl_addr_offset), dlp->bind_ack.dl_addr_length, addr);
...
/* ARGSUSED */
printdlunbindreq(dlp) union DL_primitives *dlp;
{ (void)printf("DL_UNBIND_REQ\n"); }

printdlsubsbindreq(dlp) union DL_primitives *dlp;
{
...
printdlsubsbindack(dlp) union DL_primitives *dlp;
{
    u_char sap[MAXDLADDR];
...
printdlsubsunbindreq(dlp) union DL_primitives *dlp;
{
    u_char sap[MAXDLADDR];
    ...
    (void)printf("DL_SUBS_UNBIND_REQ:  subs_sap_offset %d sub_sap_length %d\n",
                 dlp->subs_unbind_req.dl_subs_sap_offset, dlp->subs_unbind_req.dl_subs_sap_length);
    (void)printf("sap %s\n", sap);
}

printdlunitdatareq(dlp) union DL_primitives *dlp;
{
    u_char addr[MAXDLADDR];
...
printdlunitdataind(dlp) union DL_primitives *dlp;
{
...
printdluderrorind(dlp) union DL_primitives *dlp;
{
    u_char addr[MAXDLADDR];
...
printdltestreq(dlp) union DL_primitives *dlp;
{
    u_char addr[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->test_req.dl_dest_addr_offset), dlp->test_req.dl_dest_addr_length,
                 addr);
...
printdltestind(dlp) union DL_primitives *dlp;
{
    u_char dest[MAXDLADDR];
    u_char src[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->test_ind.dl_dest_addr_offset), dlp->test_ind.dl_dest_addr_length,
                 dest);
    addrtostring(OFFADDR(dlp, dlp->test_ind.dl_src_addr_offset), dlp->test_ind.dl_src_addr_length,
                 src);
...
printdltestres(dlp) union DL_primitives *dlp;
{
    u_char dest[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->test_res.dl_dest_addr_offset), dlp->test_res.dl_dest_addr_length,
                 dest);
...
printdltestcon(dlp) union DL_primitives *dlp;
{
    u_char dest[MAXDLADDR];
    u_char src[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->test_con.dl_dest_addr_offset), dlp->test_con.dl_dest_addr_length,
                 dest);
    addrtostring(OFFADDR(dlp, dlp->test_con.dl_src_addr_offset), dlp->test_con.dl_src_addr_length,
                 src);
...
printdlxidreq(dlp) union DL_primitives *dlp;
{
    u_char dest[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->xid_req.dl_dest_addr_offset), dlp->xid_req.dl_dest_addr_length,
                 dest);
...
printdlxidind(dlp) union DL_primitives *dlp;
{
    u_char dest[MAXDLADDR];
    u_char src[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->xid_ind.dl_dest_addr_offset), dlp->xid_ind.dl_dest_addr_length,
                 dest);
    addrtostring(OFFADDR(dlp, dlp->xid_ind.dl_src_addr_offset), dlp->xid_ind.dl_src_addr_length,
                 src);
...
printdlxidres(dlp) union DL_primitives *dlp;
{
    u_char dest[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->xid_res.dl_dest_addr_offset), dlp->xid_res.dl_dest_addr_length,
                 dest);
...
printdlxidcon(dlp) union DL_primitives *dlp;
{
    u_char dest[MAXDLADDR];
    u_char src[MAXDLADDR];

    addrtostring(OFFADDR(dlp, dlp->xid_con.dl_dest_addr_offset), dlp->xid_con.dl_dest_addr_length,
                 dest);
    addrtostring(OFFADDR(dlp, dlp->xid_con.dl_src_addr_offset), dlp->xid_con.dl_src_addr_length,
                 src);
...
printdludqosreq(dlp) union DL_primitives *dlp;
{
 ...
 * Return string.
 */
addrtostring(addr, length, s) u_char *addr;
u_long length;
u_char *s;
{
    int i;
...
char *dlservicemode(servicemode)
u_long servicemode;
{
...
strioctl(fd, cmd, timout, len, dp) int fd;
int cmd;
int timout;
int len;
char *dp;
