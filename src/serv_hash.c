...
int serv_hash_compare(hash_type * map, void *a, void *b) {
...
int serv_hash_hash(hash_type * map, void *key) {
    ip_service *serv = (ip_service *) key;
    return serv->protocol % hash_table_size;
}

void *serv_hash_copy_key(hash_type * map, void *orig) {
    ip_service *copy;
...
void serv_hash_delete_key(hash_type * map, void *key) {
    map->logAll(map);
    free(key);
}
